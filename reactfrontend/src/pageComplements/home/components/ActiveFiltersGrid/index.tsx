...
import {
  StyledMdOutlineClear,
  Container,
  FilterButton as FilterButtonContainer,
  TextButton,
  Title,
  ContentTop,
} from "./styles";
    ...
    <Container>
      <ContentTop>
        ...
        <Tooltip label="Limpar filtros">
          ...
          >
            <StyledMdOutlineClear size={25} />
          </div>
        </Tooltip>
      </ContentTop>

      <Wrap spacingY="10px" spacingX="20px">
        {filtersActiveds.map((item, index) => (
          <FilterButton key={item} item={item} index={index} />
        ))}
      </Wrap>
    </Container>
  );
...
export default ActiveFiltersGrid;
